...
from app.models import Notification, NotificationType, User, WarningLevel
...
def send_daily_missed_task_email(db: Session, user: User, target_date) -> bool:
    ...
    if not plan["missed_tasks"]:
        return False
    html = f"""
    <h2>FinalPlanner Monk Mode Daily Alert</h2>
    ...
    <h3>Missed mandatory blocks</h3>
    <ul>{"".join(f"<li>{task}</li>" for task in plan["missed_tasks"])}</ul>
    ...
    """
    sent = send_email(f"Monk Mode Alert: {plan['warning_level']} - {target_date}", html)
