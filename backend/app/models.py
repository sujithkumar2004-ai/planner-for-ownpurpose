...
class DailyCheckIn(Base):
    ...
    user_id: Mapped[int] = mapped_column(Integer, index=True)
    log_date: Mapped[date] = mapped_column(Date, index=True)
    wake_up_time: Mapped[str | None] = mapped_column(String(20), nullable=True)
    sleep_time: Mapped[str | None] = mapped_column(String(20), nullable=True)
    study_hours_completed: Mapped[float] = mapped_column(Float, default=0.0)
    gym_completed: Mapped[bool] = mapped_column(Boolean, default=False)
    ...
    todays_win: Mapped[str | None] = mapped_column(Text, nullable=True)
    todays_failure: Mapped[str | None] = mapped_column(Text, nullable=True)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
