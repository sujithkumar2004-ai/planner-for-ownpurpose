...
import { apiFetch, Warning } from "@/lib/api";
import { levelClass } from "@/lib/utils";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

export default function WarningsPage() {
  const { data, error, isLoading } = useSWR<Warning[]>("/warnings", apiFetch);
    ...
    <div className="grid gap-5">
      ...
      <Card>
        ...
        <CardContent className="grid gap-3">
          {data.map((warning) => <div key={warning.id} className={`rounded-md border-l-4 bg-muted/40 p-3 ${levelClass(warning.level)}`}><p className="font-semibold">{warning.level}</p><p className="text-sm text-foreground">{warning.message}</p></div>)}
